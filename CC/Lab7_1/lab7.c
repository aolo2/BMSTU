...
print_error(fragment *coords)
{
  ...
  for (uint32_t i = 0;
    i < (27 + count_digits(coords->starting.row) + 4 + count_digits(coords->starting.column) + 4);
    ++i)
  {
    printf(" ");
  }
...
parse(scanner sc)
{
  ...
  add_root(&result, init_tree(false), n_string(GRAMMAR));
  ...
  while (symbol != EOP)
  {
    ...
    else
    {
      ...
      {
        int32_t rhs_symbol = rules.data[i];
        uint32_t child_idx = add_node(
          &result,
          init_tree(is_terminal(rhs_symbol)),
          symbol_string(rhs_symbol),
          active_node);

        push_back(&magazine, create_s64(rhs_symbol, child_idx));
      }
    }
...
main(int argc, char **argv)
{
  ...
  forest res = parse(init_scanner(text, strlen(text)));
  if (!res.error)
  {
    printf("* Parsing finished successfully, printing the parse tree\n");
    print_forest(&res);
  }
