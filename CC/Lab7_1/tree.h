...
static uint32_t
_add_node(forest *f, tree node, char *name)
{
  ...
  uint32_t name_len = (strlen(name) + 1) * sizeof(char);

  node.idx = f->tree_head;
  f->nodes[f->tree_head] = node;
  f->names[f->tree_head] = (char *) malloc(name_len);
...
static void
add_root(forest *f, tree node, char *name)
{
  uint32_t idx = _add_node(f, node, name);
  assert(idx == 0);
}

static uint32_t
add_node(forest *f, tree node, char *name, int64_t parent)
{
  uint32_t idx = _add_node(f, node, name);
