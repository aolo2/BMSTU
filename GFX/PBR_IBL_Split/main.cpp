...
int main() {
    ...
    Shader approx("GLSL/quad.vert", "GLSL/approx.frag");
    ...
    PBR.setUniform("lightPos", lightPos);
    PBR.setUniform1i("irradianceMap", 0);
    PBR.setUniform1i("lut", 1);
    ...
    float *data = stbi_loadf("Textures/Skybox/Pisa/Pisa.hdr", &width, &height, &nrComponents, 0);
    GLuint hdrTexture;
    ...
    // Irradiance map generation (first term of split-sum)
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB16F, 256, 256, 0, GL_RGB, GL_FLOAT, nullptr);
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        PBR.setUniform("albedo", glm::vec3(1.0f));
        ...
        for (int i = 0; i < 6; i++) {
            ...
            PBR.setUniform("roughness", (float) i / 5.0f); 
        ...
        PBR.setUniform("metallic", 0.0f);
        PBR.setUniform("albedo", glm::vec3(0.0f));
